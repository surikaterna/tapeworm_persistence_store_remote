  ...
  if (streamId) {
    self._tapeworm.loadSnapshot(streamId).then(function (snapshot) {
      if (includeSubsequentCommits) {
        ...
        self._tapeworm._queryStream(streamId).then(function (result) {
          if (version > 0) {
            var startCommitId = 0;
            var foundEvents = 0;
            for (var i = 0; i < result.length; i++) {
              ...
              startCommitId++;
              if (foundEvents >= version) {
                break;
              }
            }
            var tooMany = foundEvents - version;
          ...
          socket.emit("/tw/response", { i: request.i, p: { snapshot: snapshot, commits: result } });
        });
      } else {
